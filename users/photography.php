...
error_reporting(0);
  ...
  $user = new User;
  if ($user->isLoggedIn()) { 
    if ($user->data()->twitter != null OR $user->data()->country !=null) {?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    ...
    <section id="main-content">
      <section class="wrapper site-min-height"  id="dynamic">
        ...
        <div class="row mt gallery js-flickity" data-flickity-options='{ "wrapAround": true ,"imagesLoaded": true,  "pageDots": false, "autoPlay": true}'>
          <?php
            $img = DB::getInstance()->query("SELECT * FROM images WHERE author = '$email' AND genre='photography' ");
            if (!$img->count()) {
